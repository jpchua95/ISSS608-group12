...
ggplot(dengue_aggregated, aes(x = Onset_Year, y = Count, fill = Age_Group)) +
  ...
  theme_minimal() +
  labs(
    title = "Dengue Cases Over Time by Region",
    x = "Year",
    y = "Case Count",
    fill = "Age Group"
  ) +
...
```{r}
cases_by_year <- dengue_12yrs %>%
  group_by(Onset_Year, Imported_Case) %>%
  summarize(Count = n(), .groups = "drop")

cases_by_year$Onset_Year <- as.factor(cases_by_year$Onset_Year)

ggplot(cases_by_year, aes(x = Onset_Year, y = Count, color = Imported_Case, group = Imported_Case)) +
  geom_line(size = 1.5) + 
  geom_point(size = 3) +   
